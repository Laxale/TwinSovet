...
namespace StickyWindows {
    ...
    public class StickyWindow : NativeWindow {
        ...
        // General Stuff
        private readonly BaseFormAdapter _originalFormAdapter; // the form
        ...
        public StickyWindow(Form form) : this(new WinFormAdapter(form)) { }
        ...
        protected override void OnHandleChange() {
            if ((int)Handle != 0) {
                _globalStickyWindows.Add(_originalFormAdapter);
            } else {
                _globalStickyWindows.Remove(_originalFormAdapter);
            }
        }
        ...
        private bool DefaultMsgProcessor(ref Message m) {
            switch (m.Msg) {
                ...
                {
                    if (_originalFormAdapter.IsMaximized)
                    {
                        break;
                    }
                    _originalFormAdapter.Activate();
        ...
        private bool OnNonClientLeftButtonDown(int iHitTest, Point point) {
            var rParent = _originalFormAdapter.Bounds;
            _offsetPoint = point;
        ...
        private bool ResizeMsgProcessor(ref Message m) {
            if (!_originalFormAdapter.Capture) {
                Cancel();
                return false;
            }
            ...
            switch (m.Msg) {
                ...
                case Win32.WM.WM_KEYDOWN: {
                    if ((int)m.WParam == Win32.VK.VK_ESCAPE) {
                        _originalFormAdapter.Bounds = _formOriginalRect; // set back old size
                        Cancel();
                    }
                    break;
                }
        ...
        private void Resize(Point p) {
            p = _originalFormAdapter.PointToScreen(p);
            ...
            if ((_resizeDirection & ResizeDir.Left) == ResizeDir.Left) {
                ...
                if (_originalFormAdapter.MaximumSize.Width != 0) {
                    iNewWidth = Math.Min(iNewWidth, _originalFormAdapter.MaximumSize.Width);
                }

                iNewWidth = Math.Min(iNewWidth, SystemInformation.MaxWindowTrackSize.Width);
                iNewWidth = Math.Max(iNewWidth, _originalFormAdapter.MinimumSize.Width);
            ...
            if ((_resizeDirection & ResizeDir.Top) == ResizeDir.Top) {
                ...
                if (_originalFormAdapter.MaximumSize.Height != 0) {
                    iNewHeight = Math.Min(iNewHeight, _originalFormAdapter.MaximumSize.Height);
                }

                iNewHeight = Math.Min(iNewHeight, SystemInformation.MaxWindowTrackSize.Height);
                iNewHeight = Math.Max(iNewHeight, _originalFormAdapter.MinimumSize.Height);
            ...
            // Done !!
            _originalFormAdapter.Bounds = _formRect;
        }
        ...
        private void StartMove() {
            _formRect = _originalFormAdapter.Bounds;
            _formOriginalRect = _originalFormAdapter.Bounds; // save original position
        ...
        private void Cancel() {
            _originalFormAdapter.Capture = false;
            _movingForm = false;
            _resizingForm = false;
            _messageProcessor = _defaultMessageProcessor;
        }
